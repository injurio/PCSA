...
void* conn_handler(void *args) {
    ...
    respond_with_file(context->connFd, context->filepath);
...
void serve_http_concurrent(pthread_t threadInfo, void* args) {
    char buf[BUFSIZE];

    struct survival_bag *context = (struct survival_bag *) args;

    if (!read_line(context->connFd, buf, BUFSIZE)) {
        return;
    }
    ...
    while (read_line(context->connFd, buf, BUFSIZE) > 0) {
        if (strcmp(buf, "\r\n") == 0) {
            break;
        }
    }
    ...
    if (strcmp(method, "GET") == 0) {
        ...
        if (!strcmp(uri, "/")) {
            snprintf(filepath, BUFSIZE, "%s/index.html", context->rootFolder);
        } else {
            snprintf(filepath, BUFSIZE, "%s%s", context->rootFolder, uri);
        }
        // respond_with_file(context->connFd, filepath);
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        serve_http_concurrent(threadInfo, context);
    }
